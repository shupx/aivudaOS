...
- `config/os.yaml`：系统运行参数（如 `runtime_process_manager`、`runtime_systemd_scope`），不参与磁吸。
- `config/sys.yaml`：公用业务参数（支持增删改），参与磁吸；默认包含 `role.id=1`。
...
上传 .tar.gz / .zip
  ...
  写入 app_installation 表 + app_runtime 表
       │
       ▼
     初始化 ${AIVUDAOS_WS_ROOT}/config/apps/{app_id}/{version}.yaml
       │
       ▼
