...
export async function uninstallApp(appId, purge = false, version = null) {
  const body = { purge: Boolean(purge) }
  if (version) body.version = version
  await apiPost(`/api/apps/${encodeURIComponent(appId)}/uninstall`, body)
  await refreshApps()
}
...
export async function getAppVersions(appId) {
  const data = await apiGet(`/api/apps/${encodeURIComponent(appId)}/versions`)
...
export async function switchAppVersion(appId, version, restart = false) {
  const result = await apiPost(`/api/apps/${encodeURIComponent(appId)}/switch-version`, {
    version,
    restart,
  })
  await refreshApps()
